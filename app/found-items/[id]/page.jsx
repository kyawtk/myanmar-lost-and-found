...
import { foundItemQuery } from "@/utils/data";
...
const ItemPage = ({ params }) => {
  ...
  useEffect(() => {
    const query = foundItemQuery(params.id);
    client.fetch(query).then((data) => {
      console.log(data);
      setItem(data[0]);
    });
  ...
  return (
    <div className="innerWidth min-h-screen bg-primary ">
      {" "}
      <div
        className="text-3xl md:text-4xl text-blue-100 hover:bg-blue-100 w-fit ml-5 pt-5  rounded-full p-2 "
        onClick={() => router.back()}
      >
      ...
      <section className="flex flex-col pt-5 mb-5 pb-5 border-b-2">
        ...
        <div className="flex flex-col gap-5 md:max-w-[700px] mx-auto">
          <div className="flex flex-col gap-3 p-3 md:p-5">
            ...
            <p className="text-center text-md md:text-lg font-semibold bg-green-500 rounded-tl-md rounded-tr-md mb-[-20px] p-5 text-white">
              Description
            </p>
            <p className="min-h-[200px] bg-white  border-green-500 text-center border-4  p-2  font-semibold text-md md:text-lg break-all">
              {item?.description}
            </p>
            <div className="bg-gray-200 rounded-lg text-gray-700 font-semibold flex flex-col gap-2 p-3">
              ...
              <p className="">
                Date found: {item?.date} , {moment(item?.date).fromNow()}
              </p>
              <p className="">Category: {item?.category}</p>
            </div>
          </div>
          <div className="">
            <h2 className="text-xl md:text-2xl font-bold bg-green-500 text-white  p-2 rounded-lg flex flex-col gap-3 justify-center items-center">
              Yours? Contact me. <MdArrowDownward />
            </h2>
            <div className="p-2 my-3 text-center  bg-gray-200 font-bold text-lg">
              <p className="underline decoration-green-500 decoration-2 ">
                Telephone☎️: {item?.contact?.tel}
              </p>
              <p className="underline decoration-green-500 decoration-2">
